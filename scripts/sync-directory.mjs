import {memberBiography} from '../server/member-biography.mjs';
...
try{
 ...
 for(let skip=0;skip<10000;skip+=500){
  ...
  for(const[k,v]of Object.entries({'$format':'json','$filter':"Language eq 'FR'",'$orderby':'ID asc','$top':'500','$skip':String(skip),'$select':'ID,PersonNumber,FirstName,LastName,Active,CantonAbbreviation,CouncilName,ParlGroupName,Party,PartyName,DateJoining,DateElection,DateLeaving,Mandates'}))url.searchParams.set(k,v);
 ...
 const history=[];
 for(let skip=0;skip<30000;skip+=500){const u=new URL('https://ws.parlament.ch/odata.svc/MemberCouncilHistory');for(const[k,v]of Object.entries({'$format':'json','$filter':"Language eq 'FR'",'$select':'ID,PersonNumber,Council,CouncilName,CantonAbbreviation,DateJoining,DateLeaving','$orderby':'ID asc','$top':'500','$skip':String(skip)}))u.searchParams.set(k,v);const response=await fetch(u,{signal:AbortSignal.timeout(45000)});if(!response.ok)throw Error('HISTORY_UNAVAILABLE');const raw=await response.text(),j=JSON.parse(raw),page=j.d?.results||j.d;if(!Array.isArray(page))throw Error('INVALID_HISTORY');writeFileSync('data/parliament/raw/'+hash(raw)+'.json',raw);history.push(...page);if(page.length<500)break;if(skip===29500)throw Error('HISTORY_PAGE_LIMIT');}
 ...
 for(const[id,members]of groups){
  ...
  Object.assign(payload,image,memberBiography(r,history.filter(h=>String(h.PersonNumber)===id)));delete payload.sha256;delete payload.retrievedAt;delete payload.sourceUrl;
