...
Build with `VITE_PUBLIC_PATH=/Switzerland/`, then run `scripts/prepare-swiss-release.mjs`. It rejects a root-path build and emits both the public-data and backend archives, checksums and URLs in `release.json`. Map `appArchiveUrl/appArchiveSha256` to `APP_ARCHIVE_URL/APP_ARCHIVE_SHA256`, and the public-data values to `PUBLIC_DATA_URL/PUBLIC_DATA_SHA256` in the protected hosting environment. The release uploader accepts destinations under the two Swiss mounts only. Windows cannot create distinct `Switzerland` and `switzerland` directories: the local `switzerland-alias` staging folder is explicitly mapped to the lowercase redirect destination.
...
Final live boundary checks also passed: OAuth cancellation returns 303 to the Swiss dashboard with a Secure cookie, cross-origin writes return 403, the representative profile returns 200, and the visual provider returns 400 indexed chunks with four matches.
