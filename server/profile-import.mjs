...
export async function syncPerson(store,id,{fetchImpl=fetch,rawDir='data/parliament/raw'}={}){
 ...
 const [members,contacts,occupations,committees]=await Promise.all([
  ...
  query('PersonOccupation',{'$select':'ID,Language,PersonNumber,OccupationName,Employer,JobTitle,StartDate,EndDate'}),query('MemberCommittee',{'$select':'ID,Language,PersonNumber,CommitteeName,CommitteeFunctionName'})]);
 ...
 const portrait='https://www.parlament.ch/sitecollectionimages/profil/original/'+id+'.jpg';let portraitUrl=null;
 try{const img=await fetchImpl(portrait,{method:'HEAD',signal:AbortSignal.timeout(10000)});if(img.ok&&img.headers.get('content-type')?.startsWith('image/'))portraitUrl=portrait;}catch{}
