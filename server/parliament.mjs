...
export function openParliament(file='data/parliament.sqlite'){
 ...
 const listBusinesses=()=>{const counts=new Map();for(const s of all('speech'))counts.set(s.businessId,(counts.get(s.businessId)||0)+1);return all('business').map(b=>({...b,passageCount:counts.get(b.id)||0})).sort((a,b)=>(b.modified||'').localeCompare(a.modified||''));};
 const speeches=()=>all('speech');const votes=()=>all('vote');const votings=()=>all('voting');
 const people=()=>{const map=new Map(all('person').map(p=>[String(p.id),p]));for(const v of votings())if(v.personId&&!map.has(v.personId))map.set(v.personId,{id:v.personId,name:v.personName,canton:v.canton,group:v.group,profileCoverage:'vote-record-only'});for(const s of speeches())if(s.personId&&!map.has(s.personId))map.set(s.personId,{id:s.personId,name:s.speaker,group:s.group,profileCoverage:'speech-record-only'});return [...map.values()].sort((a,b)=>a.name.localeCompare(b.name));};
 ...
 return {db,all,get,listBusinesses,people,search,speeches,votes,votings,readDebate:filters=>readDebate(db,decode,filters),passageContext:id=>passageContext(db,decode,id),
  ...
  business:id=>{const b=get('business',id);return b?{...b,timeline:all('event').filter(e=>e.businessId===String(id)).sort((a,b)=>(a.date||'').localeCompare(b.date||'')),votes:votes().filter(v=>v.businessId===String(id)),speeches:speeches().filter(s=>s.businessId===String(id)),popularVote:id==='20230073'?{date:'2025-09-28',status:'completed',result:'accepted',sourceUrl:'https://www.admin.ch/en/e-id-act'}:null}:null;},
  person:id=>{const p=people().find(p=>String(p.id)===String(id));if(!p)return null;const records=votings().filter(v=>v.personId===String(id)).sort((a,b)=>(b.date||'').localeCompare(a.date||''));return {...p,partyBackground:partyCatalog[p.partyId]||null,votes:records,speeches:speeches().filter(s=>s.personId===String(id)),coverage:'Imported roll calls only. Missing records are not abstentions. Party/group labels on votes describe the time of that vote.',decisionCounts:records.reduce((o,v)=>(o[v.decisionText]=(o[v.decisionText]||0)+1,o),{})};},
  close:()=>db.close()};
}
