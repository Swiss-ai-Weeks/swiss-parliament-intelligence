...
export function openParliament(file='data/parliament.sqlite'){
 ...
 // All passages spoken in a date range (a sitting day or a whole session), via the date index.
 const speechesBetween=(from,to,limit=20000)=>db.prepare(`SELECT * FROM records INDEXED BY records_speech_date WHERE kind='speech' AND json_extract(payload,'$.date')>=? AND json_extract(payload,'$.date')<? LIMIT ?`).all(String(from),String(to),limit).map(decode);
 ...
 return {db,all,get,listBusinesses,people,search,searchCandidates,speeches,speechesWhere,speechCounts,votingCounts,votingsFor,speechDaysSince,speechesBetween,votes,votings,readDebate:filters=>readDebate(db,decode,filters),passageContext:id=>passageContext(db,decode,id),
