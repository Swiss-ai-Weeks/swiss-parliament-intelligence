...
export function speechEvidence(s){return {id:'parl-'+s.id,text:s.text,language:s.language,kind:'document',attribution:`${s.speaker} · ${s.date||'date unavailable'} · ${s.speakerFunction||s.council||''}`,source:{url:s.officialUrl,title:'Official Bulletin'},reviewState:s.reviewState};}
...
export async function answerParliament(store,input,env,fetchImpl=fetch){
 ...
 const key=JSON.stringify([input.question,input.language,input.businessId,input.personId,input.passageId,filters,env.INFERENCE_MODEL,env.DEMO_REPLAY_FILE,collection?.map(s=>s.sha256)]);
 ...
 let passages=store.search(input.question,scope).filter(s=>s.text.length<7000&&inScope(s)&&(!filters.stage||store.get?.('business',s.businessId)?.statusGroup===filters.stage)),retrieval={method:'lexical',translatedQueries:[]};
