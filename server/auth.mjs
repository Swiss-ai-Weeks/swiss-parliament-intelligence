...
export function createAuth(env=process.env,fetchImpl=fetch,options={}){
 ...
 async providers(){if(!configured)return {email:false,google:false};try{const s=await call('settings');return {email:s.external?.email!==false,google:s.external?.google===true};}catch{return {email:true,google:false};}},
 ...
 async oauth(){const f=flow('oauth');const params=new URLSearchParams({provider:'google',redirect_to:callback(),code_challenge:f.challenge,code_challenge_method:'s256'});return {id:f.id,url:env.SUPABASE_URL+'/auth/v1/authorize?'+params};},
